 ...
 */
/**
 * Detect if we are running in a cloud/serverless environment where
 * native DNS works fine (Vercel, Railway, Render, etc.)
 * On these platforms the ISP DNS block is not an issue, so we skip
 * the custom resolver and just use native fetch.
 */
...
async function patchedFetch(url, options = {}) {
  // On Vercel / any cloud host — native fetch resolves Neon just fine
  if (IS_CLOUD) {
